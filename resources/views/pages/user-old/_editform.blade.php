<div class="col-md-6">
  ...
  <div class="form-group">
    ...
    <input type="text" name="email" id="email" placeholder="Ketik Email" value="{{ old('email', $user->email) }}" class="form-control @error('email') is-invalid @enderror">
  ...
  <div class="form-group">
    ...
    <select name="role" id="role" class="form-control @error('role') is-invalid @enderror" data-minimum-results-for-search="Infinity" required>
      ...
      @foreach (['admin','user'] as $item)
        <option {{ old('role', $user->role) == $item ? 'selected' : '' }}>{{ $item }}</option>
      @endforeach
    </select>
  ...
  <div class="form-group">
    ...
    @error('abc') <span class="invalid-feedback">{{ $message }}</span> @enderror
  </div>
</div>
