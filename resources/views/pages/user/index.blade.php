...
@section('content')
    <x-main-content>
        <x-section-header title="Data {{ $roleFormatted }}" :btnBack="false"></x-section-header>

        <div class="section-body">
            <h2 class="section-title">Index</h2>
            <div class="card">
                <div class="card-header justify-content-between">
                    <a href="{{ route('user.create', $role) }}" class="btn btn-primary btn-icon icon-left">
                        <i class="fas fa-plus pe-5"></i>
                        Tambah {{ $roleFormatted }}
                    </a>
                            ...
                            <tbody>
                                @foreach ($users as $item)
                                    <tr>
                                        ...
                                        <td class="oneline">
                                            <div class="">
                                                <a href="{{ route('user.show', [$role, $item->id]) }}" class="btn btn-sm btn-success btn-icon icon-left">
                                                    <i class="fas fa-eye"></i>
                                                    Show
                                                </a>
                                                <a href="{{ route('user.edit', [$role, $item->id]) }}" class="btn btn-sm btn-warning btn-icon icon-left">
...
@section('js')
    ...
    <script>
        $('.btn-delete').on('click', function() {
            ...
            $('#form-delete').attr('action', '/user/{{ $role }}/' + $(this).data('id'));
